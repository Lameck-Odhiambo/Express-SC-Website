...
// 		$this->output->set_header('Pragma: no-cache');
	...
	function student_add()
	{
		if ($this->session->userdata('admin_login') != 1)
            redirect(base_url(), 'refresh');
        $trail=count($this->crud_model->get_all())+1;
        $page_data['adm']=date('Y').'-00'.$trail;
  ...
  function student_profile($student_id)
  {
    ...
    function student($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            $data['email']        = substr(md5(rand(0, 1000000)), 0, 7).'@expresssocceracademy.com';
            $data['password']     = sha1('student');


            // if($this->input->post('parent_id') != null){
            //     $data['parent_id']    = implode(",",$this->input->post('parent_id'));
            // }
            ...
            if($validation == 1){
               ...
               foreach ($_POST['parent_id'] as   $value) {
                   $data3['student_id']=$student_id;
                   $data3['parent_id'] = $value;
                   $this->db->insert('parent_assignment', $data3);
        ...
        if ($param1 == 'do_update') {
           ...
           // $data['parent_id']      = $this->input->post('parent_id');
                ...
                $this->db->delete('parent_assignment');
                foreach ($_POST['parent_id'] as   $value) {
    ...
    function parent($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            if($validation == 1){
                ...
                $password ="username: ". $data['email']." password:".$this->input->post('password');
                $this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
                $this->email_model->account_opening_email('parent', $data['email'], $password); //SEND EMAIL ACCOUNT OPENING EMAIL
                 $this->sms_model->send_msg_via_africaStalking('Welcome to Express Soccer Academy', $data['phone']);
            }
            else{
                $this->session->set_flashdata('error_message' , get_phrase('this_email_id_is_not_available'));
            }
    ...
    function goals_manage()
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect(base_url(), 'refresh');
        $page_data['page_name']  =   'goals_manage';
        $page_data['page_title'] = 'manage goals';
        $this->load->view('backend/index', $page_data);
    }
