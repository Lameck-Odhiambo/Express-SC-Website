                  ...
                  <div class="col-md-6">
                    <?php echo form_open(base_url() . 'index.php?parents/invoice/' . $row['student_id'] . '/make_payment');?>
                    	<input type="hidden" name="invoice_id" value="<?php echo $row2['invoice_id'];?>" />
                    		 <?php if($row2['status'] == 'paid'):?>Thanks for your complete payment <?php else: ?> <a href="<?php echo base_url(); ?>index.php?parents/message " class="btn btn-info">  Contact admin to make payment </a>
                             <?php endif;?> 
